...
class TileDrawState(EditorState):
	def __init__(self, room: EditorRoom, shared_state: SharedEditorState, action_queue: EditorActionQueue, tile_selection_info: TileSelectionInfo):
		...
		self.button_frame.add_element(Button((10, 10), Common.get_value("image_res"), "draw_tool_button", self.set_tool, TileTools.DRAW, 0, size=(None, 70)))
		self.button_frame.add_element(Button((10, 0), Common.get_value("image_res"), "draw_tool_button", self.set_tool, TileTools.FILL, 1, size=(None, 70)), add_on_to_previous=(True, False), align_with_previous=(False, True))
